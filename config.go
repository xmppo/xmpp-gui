...
func ParseConfig(filename string) (c *Config, err error) {
	...
	for j := range c.Accounts {
		for i, known := range c.Accounts[j].KnownFingerprints {
			c.Accounts[j].KnownFingerprints[i].fingerprint, err = hex.DecodeString(known.FingerprintHex)
			if err != nil {
...
func (c *Config) Save() error {
	...
	for j := range c.Accounts {
		for i, known := range c.Accounts[j].KnownFingerprints {
			c.Accounts[j].KnownFingerprints[i].FingerprintHex = hex.EncodeToString(known.fingerprint)
		}
	}
...
func (a *Account) ShouldEncryptTo(uid string) bool {
	if a.AlwaysEncrypt {
		return true
	}
